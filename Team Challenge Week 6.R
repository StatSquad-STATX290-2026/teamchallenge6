...
# load package 
library(tidyr)     # used to separate poll_info into multiple columns
library(stringr)   # stringr: Used for manipulating and cleaning character strings
library(dplyr)     # dplyr: Used for data manipulation such as mutate(), select(), rename(), relocate() and bind_rows()
library(lubridate) # lubridate: Used for working with dates
library(ggplot2)   # ggplot2: Used later for exploratory visualisation
library(readr)     # readr: Used to import CSV files
...
glimpse(polls_2012_clean) # Check the structure and variable types
summary(polls_2012_clean) # Check summary statistics and identify unusual values
...
# The original script explicitly says that intermediate cleaning checks are not performed.
...
# 3. Checking that polling percentages/proportions are within sensible ranges.

# 4. Checking whether sample_size contains impossible or unusual values.

# 5. Checking state naming consistency.

# 6. Checking that date-derived variables have sensible values.

# 7. Checking whether categorical variables need to be converted to factors before modelling.
...
# DO NOT use Dem or Rep as predictors of Dem_lead. Why?

# => Dem_lead was literally defined using:

# Dem > Rep
# If we use Dem and Rep as predictors,
# the model already has the exact information used to create the outcome.
# This would cause severe data leakage and make the model uninformative for prediction.
...
# EXPLAIN:
# This checks whether poll sample size appears related to whether the Democratic candidate is leading.
...
# IMPORTANT:
# This is a sensible STARTER model using variables definitely present in the supplied cleaning script.
# Before choosing a final model, inspect the full dataset because there may be additional poll characteristics worth using.
...
# LIMITATION 2:
# Dem_lead only records whether Democratic support is greater than Republican support.
# It ignores the SIZE of the polling lead and does not directly account for sampling uncertainty.
#
# A poll with:
# Dem = 0.451, Rep = 0.450
# receives the same Dem_lead = 1 outcome as: Dem = 0.60, Rep = 0.30
